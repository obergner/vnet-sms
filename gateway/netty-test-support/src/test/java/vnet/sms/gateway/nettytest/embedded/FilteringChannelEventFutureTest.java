...
public class FilteringChannelEventFutureTest {

	@Test
	public final void assertThatCancelReturnsFalse() {
		final FilteringChannelEventFuture<ChannelEvent> objectUnderTest = new FilteringChannelEventFuture<ChannelEvent>(
		        ChannelEventFilters.ofType(MessageEvent.class));
		assertFalse(
		        "cancel() should always return false as FilteringChannelEventFuture is not cancellable",
	...
	public final void assertThatGetReturnsMatchingMessageEventIfFutureCompletesSuccessfully()
		...
		assertEquals(
		        ...
		        matchingMessageEvent, objectUnderTest.get());
	}
	...
	public final void assertThatTimedGetThrowsExecutionExceptionIfFutureCompletesUnsuccessfully()
		...
		objectUnderTest.get(200L, MILLISECONDS);
	}
	...
	public final void assertThatIsCancelledReturnsFalse() {
		final FilteringChannelEventFuture<ChannelEvent> objectUnderTest = new FilteringChannelEventFuture<ChannelEvent>(
		        ChannelEventFilters.ofType(MessageEvent.class));
		assertFalse(
		        "isCancelled() should always return false as FilteringChannelEventFuture<ChannelEvent> is not cancelable",
	...
	@Test
	public final void assertThatWhenAccessedConcurrentlyOnlyOneThreadGetsToCompleteFilteringChannelEventFuture()
		...
		assertEquals(
		        "Our FilteringChannelEventFuture<ChannelEvent> should have been completed exactly once by one thread",
		...
		try {
			...
			assertEquals(
			        "FilteringChannelEventFuture should have stored the MessageEvent set by the Thread that saw 'true' returned from onMessageEvent(...)",
			        theCompletion.channelEvent.getMessage(), MessageEvent.class
			                .cast(matchedMessageEvent).getMessage());
		} catch (final ExecutionException e) {
			assertEquals(
			        "FilteringChannelEventFuture should have stored the Exception set by the Thread that saw 'true' returned from onExceptionEvent(...)",
	...
	private static final class SuccessfulCompletor implements
		...
		private final FilteringChannelEventFuture<ChannelEvent>	objectUnderTest;
		...
		SuccessfulCompletor(
		        final FilteringChannelEventFuture<ChannelEvent> objectUnderTest,
