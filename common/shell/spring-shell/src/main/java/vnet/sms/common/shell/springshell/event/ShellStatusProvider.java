 ...
 * 
 * <p>
 * Implementations are not required to provide any guarantees with respect to
 * the order in which notifications are delivered to listeners.
 * 
 * <p>
 * Implementations must permit modification of the listener list, even while
 * delivering event notifications to listeners. However, listeners do not
 * receive any guarantee that their addition or removal from the listener list
 * will be effective or not for any event notification that is currently
 * proceeding.
 * 
 * <p>
 * Implementations must ensure that status notifications are only delivered when
 * an actual change has taken place.
 * 
 * @author Ben Alex
 * @since 1.0
 */
