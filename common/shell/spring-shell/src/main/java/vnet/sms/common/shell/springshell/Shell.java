...
import vnet.sms.common.shell.springshell.event.ShellStatusProvider;
...
public interface Shell extends ShellStatusProvider, ShellPromptAccessor {
	 ...
	 * ideally be displayed in a consistent screen location by the shell
	 * implementation.
	 * 
	 * <p>
	 * An implementation may allow multiple messages to be displayed
	 * concurrently. So an implementation can determine when a flash message
	 * replaces a previous flash message, callers should allocate a unique
	 * "slot" name for their messages. It is suggested the class name of the
	 * caller be used. This way a slot will be updated without conflicting with
	 * flash message sequences from other slots.
	 * 
	 * <p>
	 * Passing an empty string in as the "message" indicates the slot should be
	 * cleared.
	 * 
	 * <p>
	 ...
	 * 
	 * @param level
	 *            the importance of the message (cannot be null)
	 * @param message
	 *            to display (cannot be null, but may be empty)
	 * @param slot
	 *            the identification slot for the message (cannot be null or
	 *            empty)
	 */
