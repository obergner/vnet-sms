...
public abstract class HandlerUtils {
	 ...
	 * {@link Level#FINE} if it is part of vnet.sms.common.roo. Unfortunately
	 * this is needed due to a regression in JDK 1.6.0_18 as per issue ROO-539.
	 * 
	 * @param clazz
	 *            to retrieve the logger for (required)
	...
	public static Logger getLogger(final Class<?> clazz) {
		...
		if ((logger.getLevel() == null)
		        && clazz.getName().startsWith("vnet.sms.common.roo")) {
	 ...
	 * 
	 * @param logger
	 *            to introspect and replace the {@link Handler}s for (required)
	 * @param fallbackSeverity
	 *            to trigger fallback mode (required)
	 ...
	 * 
	 * @param logger
	 *            to introspect for {@link DeferredLogHandler} instances
	 *            (required)
	 * @param target
	 *            to set as the target {@link Handler}
